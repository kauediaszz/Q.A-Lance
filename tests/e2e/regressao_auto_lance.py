...
def rodar_testes_planilha():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            time.sleep(3) # Tempo para os banners da home assentarem
        ...
        try:
            ...
            # Espera até 15s para garantir que os scripts de ad carreguem os iframes
        ...
        # =========================================================
        # CT: MENU-01 - Menu de Times/Clubes
        # =========================================================
        try:
            registrar_log("⏳ [MENU-01] Menu de Times/Clubes...")
            pagina.locator("text=Futebol Nacional >> visible=true").first.hover(timeout=TIMEOUT_ELEMENTO)
            time.sleep(2) # Dá tempo para a animação do dropdown abrir completamente
            pagina.locator("text=Flamengo >> visible=true").first.click(timeout=TIMEOUT_ELEMENTO)
            
            # Espera a página do Flamengo carregar bem
            pagina.wait_for_load_state("domcontentloaded", timeout=TIMEOUT_PAGINA)
            time.sleep(3) 
            assert "flamengo" in pagina.url.lower(), "Não redirecionou para a página do time."
            
            pagina.screenshot(path=os.path.join(pasta_evidencias, "MENU-01_Menu_de_Times.png"))
            registrar_log("✅ PASSOU - Hover e clique no time funcionaram.")
        except Exception as e:
        ...
        # =========================================================
        try:
            registrar_log("⏳ [HOME-01] Carrossel de Destaques...")
            pagina.goto(URL_HOME, timeout=TIMEOUT_PAGINA)
            time.sleep(3) # Espera a Home carregar para não clicar num link errado
            
            link_destaque = pagina.locator("main").first
            link_destaque.click(timeout=TIMEOUT_ELEMENTO)
            
            expect(pagina.locator("h1").first).to_be_visible(timeout=TIMEOUT_ELEMENTO)
            time.sleep(3) # Espera a imagem da matéria carregar
            
            pagina.screenshot(path=os.path.join(pasta_evidencias, "HOME-01_Carrossel_de_Destaques.png"))
        ...
        try:
            ...
            expect(pagina.locator("h1").first).to_be_visible(timeout=TIMEOUT_ELEMENTO) 
            expect(pagina.locator("time >> visible=true").first).to_be_visible(timeout=TIMEOUT_ELEMENTO) 
        ...
        try:
            ...
            time.sleep(4) # Mídia pesada (Twitter/YouTube) demora um pouco mais a embutir
        ...
        try:
            ...
            time.sleep(3) # Tempo para os escudos dos times renderizarem na tabela
        ...
        try:
            ...
            time.sleep(5) # A agenda puxa placares de APIs externas, precisa de mais tempo
        ...
        try:
            ...
            time.sleep(4) # Espera a renderização do layout mobile
            
            pagina_mobile.locator('[data-testid="menu_burguer_icon"]').click(timeout=TIMEOUT_ELEMENTO)
            
            time.sleep(2) # IMPORTANTE: Espera a animação da gaveta do menu abrir totalmente
